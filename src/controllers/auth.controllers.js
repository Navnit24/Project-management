...
const verifyEmail = asyncHandler(async (req,res)=> {
     ...
     if(!verificationToken){
        throw new ApiError(400,"Eamail verification token is missing");
     }
       ...
       return res
          ...
          .json(
            new ApiResponse(
                200,
                {
                    acesssToken,refreshToken:newRefreshToken
                },
...
const forgotPasswordRequest = asyncHandler(async (req,res)=> {
    ...
    await sendEmail({
        ...
        mailgenContent: forgotPasswordMailgenContent(user.username,
            `${process.env.FORGOT_PASSWORD_REDIRECT_URL}/${unshashedToken}`,
        ),
      });
    ...
    return res
       ...
       .json(
         new ApiResponse(200,
            {},
            "Password resent has been sent on your email ID"
        )
       );
});
...
const resetForgotPassword = asyncHandler(async (req,res)=> {
    ...
    const user =await User.findOne({
     ...
     await user.save({alidateBeforeSave:false})
